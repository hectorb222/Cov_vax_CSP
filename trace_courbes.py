...
df_1 = importation("Desktop/Cov_vax_CSP/dataframe4.csv")
...
# extraction des listes d'infectés pour toutes les stratégies en confinement partiel
L = []
L2 = []
for i in range(6):
    n = 5*i+1
    ...
    L.append(list_infected)
    L2.append(list_mort)

dt = np.linspace(0, 365, 366)

plt.figure(1)
plt.subplot(1, 2, 1)

plt.plot(dt, L[0], label='Pas de priorisation')
plt.plot(dt, L[1], label='Personnes vulnérables')
plt.plot(dt, L[2], label='Acteurs économiques')
plt.plot(dt, L[3], label='Travailleurs essentiels')
plt.plot(dt, L[4], label='Non télétravailleurs')
plt.plot(dt, L[5], label='Très exposés')
plt.title("Evolution du nombre d'infections en confinement partiel")
plt.xlabel("Jours")
plt.legend()


plt.subplot(1, 2, 2)
plt.plot(dt, L2[0], label='Pas de priorisation')
plt.plot(dt, L2[1], label='Personnes vulnérables')
plt.plot(dt, L2[2], label='Acteurs économiques')
plt.plot(dt, L2[3], label='Travailleurs essentiels')
plt.plot(dt, L2[4], label='Non télétravailleurs')
plt.plot(dt, L2[5], label='Très exposés')
plt.title("Evolution du nombre de décès cumulés en confinement partiel")
plt.xlabel("Jours")
plt.legend()
...
x_label_list = ['Pas de priorisation', 'Plus vulnérables', 'Acteurs économiques',
